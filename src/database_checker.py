                ...
                for col_name, model_col in model_columns.items():
                    if col_name in db_columns:
                        ...
                        if (model_type_str not in db_type_str and
                                ...
                                not (model_type_str.startswith('varchar') and db_type_str.startswith(
...
def prompt_user_for_action(inconsistent_tables):
    ...
    print("tip: 如遇到无法迁移的问题，可以在备份数据库后，尝试删除Migrate文件夹和其相关的数据库表")
...
def handle_database_consistency_check(app):
    ...
    if action == 'auto_migrate':
        ...
        print(
            "2. 执行 'flask db migrate --directory alembic_migrations' 和 'flask db upgrade --directory alembic_migrations' 命令")
        print("3. 检查模型定义是否正确")
        sys.exit(0)
